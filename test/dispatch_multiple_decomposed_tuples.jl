#=
using Revise; include(joinpath("test", "dispatch_multiple_decomposed_tuples.jl"))
=#
...
function example!(dtupA, dtupB, f!, N::Int, counter)
    for i in 1:N
        ...
        # DT.dispatch(dtupB, i, f!, counter, dtupA, i) do dtB, g!, ctr, dtA, j
        #     DT.dispatch(g!, dtA, j, dtB, ctr)
        # end

        # DT.dispatch(f, dtup, i, args...)
        # This unrolls to:
        ...
        # This unrolls to:
        #
        #      tA_i = DT.outer_index(dtupA, i)
        #      tB_i = DT.outer_index(dtupB, i)
